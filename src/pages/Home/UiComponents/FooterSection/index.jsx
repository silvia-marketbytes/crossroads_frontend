...
const FooterSection = () => {
  const crossroadsData = {
    ...
    socialLinks: [
      ...
      },
      {
        ...
        href: '#',
        iconPath:
          'M12 2.163c3.204 0 3.584.012 4.85.07 1.366.062 2 Hawkinser (currentColor) 2.633.326 3.608 1.301.975.975 1.24 2.242 1.301 3.608.058 1.266.07 1.646.07 4.85s-.012 3.584-.07 4.85c-.062 1.366-.326 2.633-1.301 3.608-.975.975-2.242 1.24-3.608 1.301-1.266.058-1.646.07-4.85.07s-3.584-.012-4.85-.07c-1.366-.062-2.633-.326-3.608-1.301-.975-.975-1.24-2.242-1.301-3.608-.058-1.266-.07-1.646-.07-4.85s.012-3.584.07-4.85c.062-1.366.326-2.633 1.301-3.608.975-.975 2.242-1.24 3.608-1.301 1.266-.058 1.646-.07 4.85-.07zm0-2.163c-3.259 0-3.667.014-4.947.072-1.627.073-3.043.44-4.142 1.54-1.1 1.1-1.467 2.516-1.54 4.142-.058 1.28-.072 1.688-.072 4.947s.014 3.667.072 4.947c.073 1.627.44 3.043 1.54 4.142 1.1 1.1 2.516 1.467 4.142 1.54 1.28.058 1.688.072 4.947.072s3.667-.014 4.947-.072c1.627-.073 3.043-.44 4.142-1.54 1.1-1.1 1.467-2.516 1.54-4.142.058-1.28.072-1.688.072-4.947s-.014-3.667-.072-4.947c-.073-1.627-.44-3.043-1.54-4.142-1.1-1.1-2.516-1.467-4.142-1.54-1.28-.058-1.688-.072-4.947-.072z',
      },
      {
        label: 'LinkedIn',
        href: '#',
  ...
  return (
    <footer className="w-full">
      ...
      <div className="bg-white rounded-3xl py-8 px-4 md:px-8 flex flex-col items-center justify-center relative z-10 -mb-8 max-w-6xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 w-full">
          <h2
            className="text-2xl md:text-2xl font-semibold text-[#00334D] text-center"
          >
            ANY QUESTION OR REMARKS?
          </h2>
          <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-6">
            <button
              className="bg-[#F9920A] text-white px-4 py-2 rounded-full flex items-center space-x-2 hover:bg-[#e07b00] text-sm"
            >
            ...
            <a
              ...
              className="text-[#00334D] text-2xl font-semibold flex items-center space-x-2 whitespace-nowrap"
            >
              <svg
                className="w-5 h-5"
                fill="none"
      ...
      <div
        className="border-t border-gray-700 py-4 text-center text-gray-400 text-sm"
        style={{ backgroundColor: "#00334D" }}
      >
        <p>
          © 2024 ALL RIGHTS RESERVED BY CROSSROADS. MADE WITH PASSION BY{' '}
          <a href="#" className="text-[#F9920A] hover:underline">
