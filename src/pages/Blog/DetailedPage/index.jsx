...
const BlogDetail = () => {
  ...
  const updateThumbPosition = useCallback((activeIndex) => {
    const container = scrollRef.current;
    if (!container) return;

    const itemHeight = 40; // Approximate height of each <li> (text-base, py-1, space-y-4)
    const thumbSize = 20; // Fixed thumb size

    // Calculate thumb position for static list
    const thumbOffset = activeIndex * itemHeight + (itemHeight - thumbSize) / 2;
    setThumbTop(thumbOffset);
    setThumbHeight(thumbSize);
  }, []);

  const scrollToSection = useCallback((sectionTitle, index) => {
  ...
  useEffect(() => {
    sectionRefs.current = Array(Object.keys(checklistContent).length)
      .fill()
    ...
    const handleScroll = () => {
      ...
      // Check if at the bottom of the content
      if (contentRect.bottom <= windowHeight + 80) {
        const lastSectionIndex = Object.keys(checklistContent).length - 1;
        const lastSectionTitle = Object.keys(checklistContent)[lastSectionIndex];
        if (currentActiveIndex !== lastSectionIndex) {
          setActiveSection(lastSectionTitle);
      ...
      if (scrollingUp) {
        ...
        if (topmostSection && topmostIndex !== currentActiveIndex) {
          const sectionTitle = Object.keys(checklistContent)[topmostIndex];
          setActiveSection(sectionTitle);
        ...
        if (visibleSection && visibleIndex !== currentActiveIndex) {
          const sectionTitle = Object.keys(checklistContent)[visibleIndex];
          setActiveSection(sectionTitle);
  ...
  }, [updateThumbPosition, handleStickySidebar]);
  ...
  const instagramShareUrl = "https://www.instagram.com/crossroads_ge/";
  const linkedInShareUrl = "https://www.linkedin.com/company/crossroads-career-consultants-pvt-ltd/";
  const facebookShareUrl = "https://www.facebook.com/CrossroadsOverseasEducation";
  ...
  return (
    <div className="relative">
      ...
      <section className="px-4 sm:px-6 lg:px-20 my-6 mx-auto">
        <div className="flex flex-col lg:flex-row gap-y-6 lg:gap-x-6 max-w-6xl mx-auto">
          ...
          >
            ...
            >
              <div className="relative">
                ...
                />
                <div ref={scrollRef} className="max-h-[400px] pl-4 md:pl-6">
                  <ul className="space-y-4">
                    {Object.keys(checklistContent).map((sectionTitle, index) => (
                      <li key={index}>
                        <a
                          href={`#${slugify(sectionTitle, { lower: true, strict: true })}`}
                          className={`block text-sm md:text-base pl-3 -ml-1px py-1 transition-colors ${activeSection === sectionTitle
                            ? "text-[#F9920A] font-semibold"
                            : "border-transparent text-gray-600 hover:text-[#e07a00]"
                            }`}
                          onClick={(e) => {
                            e.preventDefault();
                            scrollToSection(sectionTitle, index);
                          }}
                        >
                          {sectionTitle}
                        </a>
          ...
          <div className="w-full lg:w-3/4">
            <div className="bg-white p-4 md:p-6 h-full rounded-lg">
              <div
                ref={contentRef}
                className="lg:pr-4 mb-8 scrollbar-visible-custom lg:pb-16 lg:overflow-y-auto"
              >
                <motion.p
                  className="text-gray-700 mb-8 mt-5 text-base md:text-lg"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                ...
                <Link
                  to="/Blogs"
                  className="inline-flex items-center text-[#F9920A] hover:text-[#e07a00] font-medium"
                >
                  ← Back to Blog
                </Link>
              </div>
      ...
      <section className="py-10 bg-[#00334D] text-white">
        <div className="px-4 sm:px-6 lg:px-20 mx-auto max-w-8xl">
