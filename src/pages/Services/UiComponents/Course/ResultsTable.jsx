...
const ResultsTable = ({ courses }) => {
  ...
  const handleDownload = (pdfUrl) => {
    console.log('Download triggered for URL:', pdfUrl);
    if (!pdfUrl) {
  ...
  const remainder = totalCourses % COURSES_PER_PAGE;
  const coursesOnLastPage = remainder > 0 ? remainder : COURSES_PER_PAGE;
  const totalPages = Math.ceil((totalCourses - coursesOnLastPage) / COURSES_PER_PAGE) + 1;

  const indexOfLastCourse = currentPage === totalPages
    ? totalCourses
    : currentPage * COURSES_PER_PAGE;
  const indexOfFirstCourse = indexOfLastCourse - (currentPage === totalPages ? coursesOnLastPage : COURSES_PER_PAGE);
              ...
              {expandedCourseId === course.id && (
                <div className="mt-4 space-y-2">
                  ...
                  <button
                    ...
                    className="px-4 py-2 border border-orange-500 text-orange-500 rounded-md hover:bg-orange-50 hover:text-orange-600 transition duration-200"
                  >
                    Apply Now
                  </button>
                </div>
            ...
            <tbody className="text-[#003050]">
              {currentCourses.map((course, index) => (
                <tr key={course.id} className="text-center bg-white hover:bg-gray-100">
                  <td className="px-3 py-6 whitespace-nowrap text-sm">{(currentPage - 1) * COURSES_PER_PAGE + index + 1}</td>
                  <td className="px-3 py-6 whitespace-nowrap text-sm font-medium">{course.courseCode}</td>
                  <td className="px-3 py-6 whitespace-nowrap text-sm">{course.course}</td>
                  ...
                  <td>
                    <button
                      onClick={() => handleApplyNow(course)}
                      className="px-5 py-1 border border-orange-500 text-orange-500 rounded-md hover:bg-orange-50 hover:text-orange-600 transition duration-200"
                    >
                      Apply Now
                    </button>
                  </td>
              ...
              {currentPage === totalPages && Array.from({ length: COURSES_PER_PAGE - currentCourses.length }).map((_, index) => (
